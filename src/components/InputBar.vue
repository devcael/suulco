...
const emit = defineEmits<{
  ...
  (e: "open-memoria-editor"): void;
}>();

const input = ref("");
const target = ref<InputDestination>("hoje");
...
const selectedSulcoLabel = computed(() => {
  ...
  return sulcoOptions.value.find((s) => s.id === selectedSulcoId.value)?.text ?? "nenhum sulco";
});
...
function onDocumentClick(e: MouseEvent) {
  if (sulcoDropdownRef.value && !sulcoDropdownRef.value.contains(e.target as Node)) {
    sulcoDropdownOpen.value = false;
  }
}
  ...
  <div class="px-spacing-page-x pt-4 pb-9 border-t border-line bg-bg">
    <div class="flex items-center justify-between mb-2.5">
      ...
      <div v-if="showSulcoSelect" class="relative" ref="sulcoDropdownRef">
        <button
          class="text-xs px-[11px] py-1 rounded-sm border font-body tracking-wide transition-all duration-150 cursor-pointer bg-transparent text-fg-subtle border-line max-w-[140px] truncate"
        ...
        >
          ...
          <button
            ...
            :class="selectedSulcoId === s.id ? 'text-fg bg-bg-subtle' : 'text-fg-subtle hover:bg-bg-subtle'"
            @click="selectSulco(s.id)"
          >{{ s.text }}</button>
          <p v-if="sulcoOptions.length === 0" class="text-xs px-3 py-1.5 text-fg-subtle italic font-body">
            nenhum sulco
          </p>
        </div>
      </div>
    ...
    <div class="flex gap-2.5 items-end">
      <textarea
        ref="textareaRef"
        class="flex-1 bg-bg-subtle border border-line rounded-md px-3.5 py-[11px] font-body text-base text-fg outline-none resize-none min-h-[44px] leading-normal overflow-hidden transition-[border-color] duration-200 focus:border-accent placeholder:text-fg-subtle placeholder:italic placeholder:text-[13px]"
        :value="input"
        @input="input = ($event.target as HTMLTextAreaElement).value; autoResize()"
        @keydown="onKeydown"
      ...
      >
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
          <path d="M7.5 12.5V2.5M7.5 2.5L3 7M7.5 2.5L12 7" stroke="#F0EAE0" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
      </button>
    </div>
  </div>
